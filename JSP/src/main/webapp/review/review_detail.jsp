...
if (review.getStatus() == 2) {//검색된 게시글이 비밀글인 경우
	...
	if (loginMember == null
	|| !loginMember.getId().equals(review.getReviewid()) && loginMember.getMemberStatus() != 9) {
		out.println("<script type='text/javascript'>");
		out.println("location.href='" + request.getContextPath() + "/index.jsp?group=error&worker=error_400'");
		out.println("</script>");
		return;
...
<div id="review_detail">
	...
	<table>
		<tr>
			<th>작성자</th>
			<td><%=review.getName()%> <%-- 로그인 상태의 사용자가 관리자인 경우 --%> <%
 ...
 %></td>
		...
		<tr>
			<th>내용</th>
			<td class="content"><%=review.getContent().replace("\n", "<br>")%>
				<br> <%
 if (review.getReviewimg() != null) {
 %> <img
				src="<%=request.getContextPath()%>/review_images/<%=review.getReviewimg()%>"
				width="200"> <%
 }
 %></td>
		</tr>
